...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'KY2099') return res.json(loghandler.invalidKey)
...
router.get('/remove', (req, res, next) => {
    ...
    if (!apikey) return res.json(loghandler.notparam)
    if (!(status && apikeyInput && email && nomorhp && name && age && country && exp)) return res.json(loghandler.notAddApiKey)
    if (apikey != 'KY2098') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != listkey.includes(apikeyInput))  return res.json(loghandler.invalidKey)
